...
**Mock data layer — this is the entire "backend" today.** All under `src/data/`:
- `stocks.ts` — ~28 real PSE tickers with price/change/sector/float/market-cap/index-weight. `getStockBySymbol()` is the lookup used everywhere.
- `holdings.ts` — seed holdings for the mock signed-in user.
- `news.ts` — static PH market news items (headline/source/summary/url); tapping a card opens the real source URL externally.
- `market-index.ts` — current PSEi index value.
- `performance.ts` — `generatePerformanceSeries(range, endValue)`, a *deterministic* seeded random-walk generator (same seed per range → stable output across renders). Reused for portfolio value history, the PSEi index chart, and individual stock price history — it's range/endValue-agnostic by design, don't fork it per use case.

Any future real market-data or news integration should replace these modules' internals while keeping their exported shapes, rather than threading fetch calls through components.

**Charts** are hand-rolled SVG (`src/components/performance-graph.tsx`, via `react-native-svg`) — a smoothed line + gradient-fill area, no charting library. `src/hooks/use-range-series.ts` centralizes the "pick a range, generate series, compute change amount/%" logic shared by the portfolio, index, and stock-detail charts — extend that hook rather than re-deriving the stats inline in a new card.

**Component layout:** `src/components/ui/` = generic, content-agnostic primitives (`primary-button`, `avatar-badge`, `change-pill`, `detail-grid`, `search-bar`, `segmented-control`, `market-status-badge`, etc.). `src/components/` (flat) = feature-specific composites that know about stocks/holdings/news (`holding-row`, `stock-row`, `news-card`, `performance-card`, `market-index-card`, `screen-header`). New reusable, domain-agnostic UI goes in `ui/`; anything that imports from `@/data` or `@/context` goes flat.
